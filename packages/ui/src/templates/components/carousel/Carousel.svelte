...
<div class={disabled ? 'pointer-events-none opacity-50' : ''} aria-disabled={disabled || undefined}>
	<Carousel.Root
	...
	>
		...
		{#if showControls}
			<Carousel.Control
				class="absolute inset-x-0 top-1/2 flex -translate-y-1/2 justify-between px-2"
			>
				<Carousel.PrevTrigger
					class="rounded-km-selector border-km-base-300 bg-km-base-100 text-km-base-content shadow-km-sm hover:bg-km-base-200 flex h-8 w-8 cursor-pointer items-center justify-center border transition-colors duration-150 disabled:cursor-not-allowed disabled:opacity-50"
				>
					<ChevronLeft size={18} />
				</Carousel.PrevTrigger>
		...
		{#if showIndicators && pageCount > 1}
			<Carousel.IndicatorGroup class="mt-3 flex items-center justify-center gap-1.5">
				{#each { length: pageCount } as _, index (index)}
					<Carousel.Indicator
						{index}
						class="rounded-km-selector bg-km-base-300 hover:bg-km-muted-content data-[current]:bg-km-primary h-2 w-2 cursor-pointer border-none transition-colors duration-150"
					/>
				{/each}
