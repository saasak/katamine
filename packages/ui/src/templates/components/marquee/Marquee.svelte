...
<div class={disabled ? "pointer-events-none opacity-50" : undefined} aria-disabled={disabled || undefined}>
  ...
  >
    ...
    {#if showEdges}
      {#if isVertical}
        <Marquee.Edge side="top" class="h-16 bg-gradient-to-b from-km-base-100 to-transparent" />
        <Marquee.Edge side="bottom" class="h-16 bg-gradient-to-t from-km-base-100 to-transparent" />
      {:else}
        <Marquee.Edge side="start" class="w-16 bg-gradient-to-r from-km-base-100 to-transparent" />
        <Marquee.Edge side="end" class="w-16 bg-gradient-to-l from-km-base-100 to-transparent" />
      {/if}
    {/if}
  </Marquee.Root>
</div>
