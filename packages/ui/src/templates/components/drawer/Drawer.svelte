...
>
  ...
  <Portal>
    ...
    />
    <Drawer.Positioner
      class="{positionerClass} z-[var(--km-z-modal)] overflow-hidden"
    >
      <Drawer.Content
        class="{contentClass} border-km-base-300 bg-km-base-100 shadow-km-lg {hasSnapPoints ? '' : 'data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in data-[state=closed]:fade-out'} {className ?? ''}"
      >
        {#if showGrabber}
        ...
        <div class="overflow-y-auto {isVertical ? 'px-6 pb-[max(1.5rem,env(safe-area-inset-bottom))]' : 'p-6 h-full'}">
          <Drawer.CloseTrigger
