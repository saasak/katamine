		...
		<TreeView.NodeProvider {node} indexPath={[...parentPath, i]}>
			{#if node.children?.length}
				<TreeView.Branch class="relative">
					<TreeView.BranchControl
						class="rounded-km-selector text-km-base-content hover:bg-km-base-200 data-[selected]:bg-km-base-200 data-[selected]:text-km-primary data-[focus-visible]:outline-km-primary flex w-full cursor-pointer items-center gap-1 px-2 py-1 text-sm transition-colors duration-150 data-[disabled]:cursor-not-allowed data-[disabled]:opacity-50 data-[focus-visible]:outline-2 data-[focus-visible]:-outline-offset-1"
					>
						<TreeView.BranchTrigger
							class="rounded-km-selector text-km-muted-content hover:bg-km-muted hover:text-km-base-content flex h-5 w-5 shrink-0 items-center justify-center transition-colors duration-150 data-[disabled]:pointer-events-none"
						>
							<TreeView.BranchIndicator
								class="transition-transform duration-150 data-[state=open]:rotate-90"
							>
			...
			{:else}
				<TreeView.Item
					class="rounded-km-selector text-km-base-content hover:bg-km-base-200 data-[selected]:bg-km-base-200 data-[selected]:text-km-primary data-[focus-visible]:outline-km-primary flex w-full cursor-pointer items-center gap-1 px-2 py-1 text-sm transition-colors duration-150 data-[disabled]:cursor-not-allowed data-[disabled]:opacity-50 data-[focus-visible]:outline-2 data-[focus-visible]:-outline-offset-1"
				>
					<TreeView.ItemText class="flex-1 pl-6 select-none">{node.label}</TreeView.ItemText>
				</TreeView.Item>
			{/if}
...
>
	{#if label}
		<TreeView.Label class="text-km-base-content mb-1 block text-sm font-medium"
			>{label}</TreeView.Label
		>
	{/if}
	<TreeView.Tree class="rounded-km-box border-km-base-300 bg-km-base-100 border p-1">
