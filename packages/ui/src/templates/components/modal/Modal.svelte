...
{#if showDrawer}
	...
	>
		...
		<Portal>
			...
			/>
			<Drawer.Positioner
				class="fixed inset-x-0 bottom-0 z-[var(--km-z-modal)] max-h-dvh overflow-hidden"
			>
				<Drawer.Content
					class="border-km-base-300 bg-km-base-100 shadow-km-lg max-h-[85dvh] w-full rounded-t-2xl border-x border-t {hasSnapPoints
						? ''
						: 'data-[state=open]:animate-in data-[state=open]:fade-in data-[state=closed]:animate-out data-[state=closed]:fade-out'} {className ??
						''}"
				>
					...
					<div class="overflow-y-auto px-6 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
						...
						{#if title}
							<Drawer.Title class="text-km-base-content text-lg font-semibold">
								{title}
							</Drawer.Title>
						{/if}
