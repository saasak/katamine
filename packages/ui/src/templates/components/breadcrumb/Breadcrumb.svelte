		...
		{#each visibleItems as item, i}
			...
			<li class="flex items-center gap-1.5">
				...
				{#if i === visibleItems.length - 1}
					<span
						class="text-km-base-content inline-flex items-center gap-1 font-medium"
						aria-current="page"
					>
				...
				{:else if item.href && !item.disabled}
					<a
						href={item.href}
						class="text-km-muted-content hover:text-km-base-content inline-flex items-center gap-1 transition-colors duration-150"
					>
						{#if item.icon}{@render item.icon()}{/if}
						{item.label}
					</a>
				{:else}
					<span
						class="text-km-muted-content inline-flex items-center gap-1 {item.disabled
							? 'opacity-50'
							: ''}"
