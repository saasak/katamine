...
<Accordion.Root
  ...
  {onFocusChange}
  {orientation}
  class="w-full divide-y divide-km-base-300 rounded-km-box border border-km-base-300 bg-km-base-100 {className ?? ''}"
>
  {#each items as item (item.value)}
    <Accordion.Item value={item.value} disabled={item.disabled}>
      <Accordion.ItemTrigger class="flex w-full cursor-pointer items-center justify-between border-none bg-transparent px-4 py-3 text-sm font-medium text-km-base-content transition-colors duration-150 hover:bg-km-base-200 disabled:cursor-not-allowed disabled:opacity-50">
        <span>{item.label}</span>
        <Accordion.ItemIndicator class="text-km-muted-content transition-transform duration-150 data-[state=open]:rotate-180">
          <ChevronDown size={16} />
        </Accordion.ItemIndicator>
      </Accordion.ItemTrigger>
      <Accordion.ItemContent class="overflow-hidden">
        <div class="px-4 py-3 text-sm text-km-base-content">
