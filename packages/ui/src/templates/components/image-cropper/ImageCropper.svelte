...
>
	<ImageCropper.Viewport class="rounded-km-box bg-km-base-200 relative h-72 w-full overflow-hidden">
		...
		>
			{#each ImageCropper.handles as position}
				<ImageCropper.Handle {position} class="absolute flex items-center justify-center">
					<div class="size-2.5 rounded-sm border border-white/50 bg-white shadow-sm"></div>
				</ImageCropper.Handle>
			{/each}
			<ImageCropper.Grid axis="horizontal" class="pointer-events-none absolute inset-0" />
			<ImageCropper.Grid axis="vertical" class="pointer-events-none absolute inset-0" />
		</ImageCropper.Selection>
	...
	<ImageCropper.Context>
		{#snippet render(api)}
			<div class="flex flex-wrap items-center gap-1.5">
				<!-- Zoom controls -->
				<div
					class="rounded-km-field border-km-base-300 bg-km-base-100 flex items-center gap-1 border p-1"
				>
					<button
						type="button"
						onclick={() => api().zoomBy(-0.1)}
						class="rounded-km-selector text-km-muted-content hover:bg-km-muted hover:text-km-base-content flex size-7 cursor-pointer items-center justify-center transition-colors duration-150"
						aria-label="Zoom out"
					>
						<ZoomOut size={15} />
					</button>
					<span class="text-km-muted-content min-w-10 text-center text-xs tabular-nums">
						{api().zoom.toFixed(1)}×
					</span>
					<button
						type="button"
				...
				>
					<button
						...
						class="rounded-km-selector flex size-7 cursor-pointer items-center justify-center transition-colors duration-150 {api()
							.flip.horizontal
							? 'bg-km-primary text-km-primary-content'
							: 'text-km-muted-content hover:bg-km-muted hover:text-km-base-content'}"
				...
				<div class="ml-auto flex items-center gap-1.5">
					...
					<button
						type="button"
						onclick={() => api().reset()}
						class="rounded-km-selector border-km-base-300 bg-km-base-100 text-km-muted-content hover:bg-km-muted hover:text-km-base-content flex size-8 cursor-pointer items-center justify-center border transition-colors duration-150"
						aria-label="Reset"
					>
						<RefreshCcw size={15} />
					</button>
					...
					{#if oncrop}
						<button
							...
							class="rounded-km-field bg-km-primary text-km-primary-content flex cursor-pointer items-center gap-1.5 px-3 py-1.5 text-sm font-medium transition-opacity duration-150 hover:opacity-90"
						>
							<Crop size={14} />
							Crop
						</button>
