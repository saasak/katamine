...
>
  <Pagination.PrevTrigger
    class="inline-flex h-9 w-9 cursor-pointer items-center justify-center rounded-km-field border border-km-base-300 bg-km-base-100 text-km-base-content transition-colors duration-[var(--km-transition-fast)] hover:bg-km-base-200 disabled:cursor-not-allowed disabled:opacity-50"
  >
    <ChevronLeft size={16} />
  </Pagination.PrevTrigger>
      ...
      {#each context.pages as p}
        {#if p.type === "page"}
          <Pagination.Item
            {type}
            value={p.value}
            class="inline-flex h-9 w-9 cursor-pointer items-center justify-center rounded-km-field border border-km-base-300 bg-km-base-100 text-sm font-medium text-km-base-content transition-colors duration-[var(--km-transition-fast)] hover:bg-km-base-200 data-[selected]:border-km-primary data-[selected]:bg-km-primary data-[selected]:text-km-primary-content"
          >
            {p.value}
          </Pagination.Item>
        {:else}
          <Pagination.Ellipsis
            ...
            class="inline-flex h-9 w-9 items-center justify-center text-km-muted-content"
          >
            <Ellipsis size={16} />
          </Pagination.Ellipsis>
        {/if}
