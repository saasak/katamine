...
>
	<PasswordInput.Control
		class="rounded-km-field bg-km-base-100 data-[invalid]:focus-within:outline-km-error flex items-center border transition-colors duration-150 focus-within:outline-2 focus-within:-outline-offset-1 data-[disabled]:cursor-not-allowed data-[disabled]:opacity-50 {invalid
			? 'border-km-error'
			: 'border-km-base-300 focus-within:border-km-primary focus-within:outline-km-primary'}"
	>
		<PasswordInput.Input
			bind:value
			{placeholder}
		...
		<PasswordInput.VisibilityTrigger
			class="text-km-muted-content hover:text-km-base-content flex cursor-pointer items-center justify-center py-2 pr-2.5 pl-1 transition-colors duration-150"
		>
