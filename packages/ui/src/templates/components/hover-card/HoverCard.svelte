...
>
	<HoverCard.Trigger>
		{#snippet child(triggerProps)}
			<span
				{...triggerProps}
				class="focus-visible:outline-km-primary inline-flex rounded-sm focus-visible:outline-2 focus-visible:outline-offset-2"
			>
				{@render trigger()}
			</span>
		{/snippet}
	...
	<Portal>
		<HoverCard.Positioner class="z-[var(--km-z-popover)]">
			<HoverCard.Content
				class="rounded-km-box border-km-base-300 bg-km-base-100 text-km-base-content shadow-km-md w-72 border p-4 text-sm transition-opacity duration-[var(--km-transition-fast)] data-[state=closed]:opacity-0 data-[state=open]:opacity-100 {className ??
					''}"
			>
				{#if arrow}
					<HoverCard.Arrow>
