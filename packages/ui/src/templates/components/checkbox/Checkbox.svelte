...
>
  <Checkbox.Control
    class="flex h-5 w-5 items-center justify-center rounded-km-selector border border-km-base-300 bg-km-base-100 transition-colors duration-150 data-[state=checked]:border-km-primary data-[state=checked]:bg-km-primary data-[state=indeterminate]:border-km-primary data-[state=indeterminate]:bg-km-primary data-[focus]:border-km-primary data-[focus]:outline-2 data-[focus]:outline-km-primary data-[focus]:-outline-offset-1 data-[invalid]:border-km-error"
  >
  ...
  {#if label}
    <Checkbox.Label class="text-sm text-km-base-content data-[disabled]:text-km-muted-content">
      {label}
    </Checkbox.Label>
  {/if}
