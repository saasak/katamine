<script lang="ts">
	...
	const basicSteps = [
		{
			...
			description:
				"This is a guided tour of the Katamine UI library. Let's explore the components together.",
		...
		},
		{
			...
			description:
				'Use the sidebar to navigate between available components. Each has live demos and a props table.',
			actions: [
	...
	const targetedSteps = [
		...
		},
		{
			...
			title: 'Live demos',
			description:
				'Each component ships with interactive demos you can explore directly on the page.',
	...
	const propsData = [
		{
			...
			type: 'StepDetails[]',
			default: '—',
			description: 'Tour steps defining content and target elements'
		},
		{
			name: 'stepId',
			type: 'string | null',
			default: '—',
			description: "Controlled step ID. Set to a step's id to open, null to close"
		},
		{
			name: 'closeOnInteractOutside',
			type: 'boolean',
			default: 'true',
			description: 'Close tour when clicking outside the content panel'
		},
		{
			name: 'closeOnEscape',
			type: 'boolean',
			default: 'true',
			description: 'Close tour on Escape key press'
		},
		{
			name: 'keyboardNavigation',
			type: 'boolean',
			default: 'true',
			description: 'Enable left/right arrow key navigation between steps'
		},
		{
			name: 'preventInteraction',
			type: 'boolean',
			default: 'false',
			description: 'Prevent interaction with the page while tour is open'
		},
		{
			name: 'spotlightOffset',
			type: '{ x: number; y: number }',
			default: '{ x: 10, y: 10 }',
			description: 'Spotlight padding around the target element (px)'
		},
		{
			name: 'spotlightRadius',
			type: 'number',
			default: '4',
			description: 'Spotlight border-radius in pixels'
		},
		{
			name: 'disabled',
			type: 'boolean',
			default: 'false',
			description: 'Disable the tour (prevents starting)'
		},
		{
			name: 'onStepChange',
		...
		},
		{
			...
			default: '—',
			description: 'Callback when tour status changes (started, dismissed, completed)'
		},
		{
			name: 'class',
			type: 'string',
		...
		{
			name: 'children',
			type: 'Snippet<[{ start: (id?: string) => void }]>',
			default: '—',
			description:
				'Render snippet for trigger content — receives { start } to programmatically begin the tour'
		}
	];
</script>
...
<div class="space-y-8">
	...
	<div>
		...
		<p class="text-km-muted-content mt-2">
			Guided step-by-step walkthrough with spotlight, backdrop, and popper-positioned tooltips.
			Built on Ark UI.
		</p>
	</div>
...
<Tour {steps}>
  ...
  {/snippet}
		...
		>
			<div class="space-y-3">
				<p id="tour-demo-heading" class="text-km-base-content text-sm font-semibold">
					← This element gets highlighted
				</p>
				<p id="tour-demo-section" class="text-km-muted-content text-sm">← This one too</p>
		...
		>
			<Tour steps={backdropSteps}>
				{#snippet children({ start })}
					<button
						onclick={() => start()}
						class="rounded-km-field border-km-base-300 text-km-base-content hover:bg-km-base-200 border px-4 py-2 text-sm font-medium transition-colors"
					>
						Toggle Backdrop
					</button>
				{/snippet}
			...
			<Tour steps={basicSteps} disabled>
				{#snippet children({ start })}
					<button
						onclick={() => start()}
						disabled
						class="rounded-km-field border-km-base-300 text-km-muted-content cursor-not-allowed border px-4 py-2 text-sm font-medium opacity-50"
					>
						Start Tour (Disabled)
					</button>
				{/snippet}
