<script lang="ts">
  ...
  const propsData = [
    { name: "items", type: "BreadcrumbItem[]", default: "—", description: "Array of breadcrumb items from root to current page" },
    { name: "separator", type: "Snippet", default: "ChevronRight icon", description: "Custom separator snippet rendered between items" },
    { name: "maxItems", type: "number", default: "0", description: "Max visible items before truncation. 0 = show all" },
    { name: "label", type: "string", default: '"Breadcrumb"', description: "Accessible aria-label for the nav element" },
    { name: "class", type: "string", default: "—", description: "Additional CSS classes on the root nav element" },
  ];
</script>
...
<div class="space-y-8">
  <div>
    ...
    <p class="mt-2 text-km-muted-content">
      Navigation breadcrumbs showing the current page's location within a hierarchy. Semantic HTML with proper WAI-ARIA attributes.
    </p>
  </div>
