<script lang="ts">
  ...
  const propsData = [
    ...
    { name: "side", type: '"start" | "end" | "top" | "bottom"', default: '"start"', description: "Scroll direction" },
    { name: "speed", type: "number", default: "50", description: "Scroll speed in pixels per second" },
    { name: "delay", type: "number", default: "0", description: "Delay before animation starts (seconds)" },
    { name: "loopCount", type: "number", default: "0", description: "Number of loops (0 = infinite)" },
    { name: "spacing", type: "string", default: '"1rem"', description: "Gap between items (CSS value)" },
    { name: "autoFill", type: "boolean", default: "false", description: "Auto-duplicate items to fill the container" },
    { name: "pauseOnInteraction", type: "boolean", default: "false", description: "Pause scrolling on hover or focus" },
    ...
    { name: "defaultPaused", type: "boolean", default: "false", description: "Initial paused state for uncontrolled mode" },
    { name: "showEdges", type: "boolean", default: "false", description: "Show gradient fade overlays at leading and trailing edges" },
    { name: "label", type: "string", default: "—", description: "Accessible label describing the scrolling content" },
    { name: "disabled", type: "boolean", default: "false", description: "Disable the marquee" },
    { name: "onPauseChange", type: "(details: { paused: boolean }) => void", default: "—", description: "Callback when pause state changes" },
    { name: "onLoopComplete", type: "() => void", default: "—", description: "Callback on each loop iteration" },
    { name: "onComplete", type: "() => void", default: "—", description: "Callback when all loops complete (finite loops only)" },
    { name: "class", type: "string", default: "—", description: "Additional CSS classes on the root element" },
    { name: "children", type: "Snippet<[T, number]>", default: "—", description: "Render snippet for each item — receives item and its index" },
  ];
</script>
...
<div class="space-y-8">
  ...
  <div>
    ...
    <p class="mt-2 text-km-muted-content">
      Continuously scrolling content strip with support for auto-fill, pause on interaction, and edge gradients. Built on Ark UI.
    </p>
  </div>
  ...
  {#snippet children(logo)}
    <div class="flex items-center gap-2 rounded-km-field border border-km-base-300 bg-km-base-100 px-4 py-2 text-sm font-medium">
  ...
  {#snippet children(logo)}
    <span class="rounded-km-selector bg-km-base-200 px-3 py-1 text-sm font-medium text-km-base-content">
      {logo.name}
    </span>
  {/snippet}
</Marquee>`}
  ...
  {#snippet children(review)}
    <div class="w-48 rounded-km-box border border-km-base-300 bg-km-base-100 p-3 shadow-km-sm">
      <p class="text-xs text-km-muted-content">{review.author}</p>
      <p class="mt-1 text-sm text-km-base-content">{review.text}</p>
    </div>
  {/snippet}
</Marquee>`}
    >
...
<Marquee items={logos} {paused} onPauseChange={(d) => paused = d.paused} label="Technology logos">
  {#snippet children(logo)}
    <span class="rounded-km-selector bg-km-primary px-3 py-1 text-sm font-medium text-km-primary-content">
      {logo.name}
    </span>
  {/snippet}
</Marquee>`}
  ...
  {#snippet children(logo)}
    <span class="rounded-km-selector bg-km-base-200 px-3 py-1 text-sm text-km-muted-content">
      {logo.name}
    </span>
  {/snippet}
</Marquee>`}
