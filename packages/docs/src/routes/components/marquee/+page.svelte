  ...
  {/snippet}
		...
		>
			<Marquee items={logos} label="Technology logos">
				{#snippet children(logo)}
					<div
						class="rounded-km-field border-km-base-300 bg-km-base-100 flex items-center gap-2 border px-4 py-2 text-sm font-medium"
					>
						<span class={logo.color}>{logo.name}</span>
					</div>
				{/snippet}
  ...
  {/snippet}
		...
		>
			<Marquee items={logos} autoFill label="Technology logos">
				{#snippet children(logo)}
					<span
						class="rounded-km-selector bg-km-base-200 text-km-base-content px-3 py-1 text-sm font-medium"
					>
						{logo.name}
					</span>
				{/snippet}
  ...
  {/snippet}
		...
		>
			<Marquee items={reviews} pauseOnInteraction showEdges speed={40} label="Customer reviews">
				{#snippet children(review)}
					<div
						class="rounded-km-box border-km-base-300 bg-km-base-100 shadow-km-sm w-48 border p-3"
					>
						<p class="text-km-muted-content text-xs">{review.author}</p>
						<p class="text-km-base-content mt-1 text-sm">{review.text}</p>
					</div>
				{/snippet}
			</Marquee>
		</DemoCard>
		...
		>
			<div class="space-y-3">
				...
				>
					{#snippet children(logo)}
						<span
							class="rounded-km-selector bg-km-primary text-km-primary-content px-3 py-1 text-sm font-medium"
						>
							{logo.name}
						</span>
					{/snippet}
  ...
  {/snippet}
		...
		>
			<Marquee items={logos} disabled label="Technology logos">
				{#snippet children(logo)}
					<span class="rounded-km-selector bg-km-base-200 text-km-muted-content px-3 py-1 text-sm">
						{logo.name}
					</span>
				{/snippet}
			</Marquee>
