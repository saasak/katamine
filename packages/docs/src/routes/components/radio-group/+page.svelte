<script lang="ts">
  ...
  const propsData = [
    { name: "items", type: "Item[]", default: "—", description: "Radio options to render. Each item has value, label, and optional disabled" },
    { name: "value", type: "string", default: "—", description: "Controlled selected value. Supports bind:value" },
    ...
    >
      <div class="space-y-2">
        ...
        <p class="text-sm text-km-muted-content">
          Selected: <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{selectedFruit}</code>
        </p>
      </div>
    </DemoCard>
