<script lang="ts">
  ...
  const propsData = [
    ...
    { name: "draggable", type: "boolean", default: "true", description: "Whether the panel can be dragged" },
    { name: "resizable", type: "boolean", default: "true", description: "Whether the panel can be resized" },
    { name: "defaultSize", type: "{ width: number; height: number }", default: "{ width: 320, height: 240 }", description: "Initial size of the panel" },
    { name: "minSize", type: "{ width: number; height: number }", default: "—", description: "Minimum size constraints" },
    { name: "maxSize", type: "{ width: number; height: number }", default: "—", description: "Maximum size constraints" },
    { name: "defaultPosition", type: "{ x: number; y: number }", default: "—", description: "Initial position of the panel" },
    { name: "closeOnEscape", type: "boolean", default: "true", description: "Close the panel when Escape is pressed" },
    { name: "persistRect", type: "boolean", default: "false", description: "Preserve panel size and position when closed" },
    { name: "gridSize", type: "number", default: "1", description: "Snap grid size in pixels for drag and resize" },
    { name: "disabled", type: "boolean", default: "false", description: "Disable all panel interactions" },
    { name: "onOpenChange", type: "(details) => void", default: "—", description: "Callback when the panel opens or closes" },
    { name: "onPositionChange", type: "(details) => void", default: "—", description: "Callback when the panel is moved via dragging" },
    { name: "onSizeChange", type: "(details) => void", default: "—", description: "Callback when the panel is resized" },
    { name: "onStageChange", type: "(details) => void", default: "—", description: "Callback when stage changes (minimized/maximized/default)" },
    { name: "trigger", type: "Snippet", default: "—", description: "Content rendered inside the trigger button" },
    { name: "children", type: "Snippet", default: "—", description: "Content rendered inside the panel body" },
    { name: "class", type: "string", default: "—", description: "Additional CSS classes on the panel content element" },
  ];
</script>
  ...
  {/snippet}
    ...
    >
      <FloatingPanel title="Notes" defaultOpen={true} defaultPosition={{ x: 40, y: 40 }}>
        ...
        {/snippet}
        {#snippet children()}
          <p class="text-km-muted-content">Drag the header to move me around. Resize from any edge.</p>
        {/snippet}
      </FloatingPanel>
    </DemoCard>
    ...
    >
      <div class="space-y-3">
        <button
          ...
          class="rounded-km-field border border-km-base-300 bg-km-base-100 px-3 py-2 text-sm font-medium text-km-base-content transition-colors duration-150 hover:bg-km-base-200"
        >
          {controlledOpen ? "Close" : "Open"} Panel
        </button>
        <FloatingPanel title="Controlled" bind:open={controlledOpen} defaultPosition={{ x: 40, y: 40 }}>
          {#snippet children()}
...
>
  ...
  {/snippet}
    ...
    >
      <FloatingPanel
        ...
        defaultOpen={true}
        defaultPosition={{ x: 40, y: 40 }}
        onPositionChange={(d) => (lastPosition = `x: ${Math.round(d.position.x)}, y: ${Math.round(d.position.y)}`)}
        onSizeChange={(d) => (lastSize = `${Math.round(d.size.width)} × ${Math.round(d.size.height)}`)}
      >
        {#snippet trigger()}Open{/snippet}
        {#snippet children()}
          <div class="space-y-1 text-xs text-km-muted-content">
            <p>Drag or resize to see events.</p>
            {#if lastPosition}<p>Position: <code class="rounded bg-km-base-200 px-1">{lastPosition}</code></p>{/if}
            {#if lastSize}<p>Size: <code class="rounded bg-km-base-200 px-1">{lastSize}</code></p>{/if}
          </div>
        {/snippet}
      </FloatingPanel>
    </DemoCard>
  ...
  {/snippet}
    ...
    >
      <FloatingPanel title="Disabled" disabled={true} defaultOpen={true} defaultPosition={{ x: 40, y: 40 }}>
        {#snippet trigger()}Open{/snippet}
        {#snippet children()}
