<script lang="ts">
  ...
  const propsData = [
    ...
    { name: "placeholder", type: "string", default: '"○"', description: "Placeholder character for each input" },
    { name: "type", type: '"numeric" | "alphanumeric" | "alphabetic"', default: '"numeric"', description: "Allowed input type" },
    { name: "mask", type: "boolean", default: "false", description: "Mask input like a password field" },
    { name: "otp", type: "boolean", default: "true", description: "Enable one-time-code autocomplete" },
    { name: "autoFocus", type: "boolean", default: "false", description: "Auto-focus the first input on mount" },
    { name: "blurOnComplete", type: "boolean", default: "false", description: "Blur inputs when all fields are filled" },
    { name: "selectOnFocus", type: "boolean", default: "true", description: "Select input value on focus" },
    ...
    { name: "pattern", type: "string", default: "—", description: "Regex pattern to validate input" },
    { name: "onValueChange", type: "(details) => void", default: "—", description: "Callback when value changes" },
    { name: "onValueComplete", type: "(details) => void", default: "—", description: "Callback when all inputs are filled" },
    { name: "onValueInvalid", type: "(details) => void", default: "—", description: "Callback when an invalid value is entered" },
    ...
    >
      <div class="space-y-2">
        ...
        <p class="text-sm text-km-muted-content">
          Value: <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{controlledValue.join("")}</code>
        </p>
      </div>
    </DemoCard>
...
/>`}
    >
      <div class="space-y-2">
        ...
        {#if completedCode}
          <p class="text-sm text-km-muted-content">
            Completed: <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{completedCode}</code>
          </p>
        {/if}
      </div>
    </DemoCard>
