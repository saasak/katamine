			...
			<Drawer direction="left" title="Navigation">
				...
				<nav class="space-y-1">
					{#each navItems as item}
						<button
							class="rounded-km-field text-km-base-content hover:bg-km-muted flex w-full items-center gap-3 px-3 py-2.5 text-sm transition-colors"
						>
							<span>{item.icon}</span>
  ...
  <p>Side panel content.</p>
		...
		>
			<Drawer direction="right" title="Item Details" description="View and edit item properties.">
				...
				<div class="space-y-4">
					<div>
						<label class="text-km-muted-content text-xs font-medium">Name</label>
						<p class="text-km-base-content text-sm">Example Item</p>
					</div>
					<div>
						<label class="text-km-muted-content text-xs font-medium">Status</label>
						<p class="text-km-base-content text-sm">Active</p>
					</div>
					<div>
						<label class="text-km-muted-content text-xs font-medium">Created</label>
						<p class="text-km-base-content text-sm">April 12, 2026</p>
					</div>
				</div>
			</Drawer>
		</DemoCard>
  ...
  <!-- scrollable content -->
		...
		>
			...
			>
				...
				<div class="space-y-2">
					{#each Array(8) as _, i}
						<div class="rounded-km-field border-km-base-300 flex items-center gap-3 border p-3">
							<div class="bg-km-muted h-10 w-10 shrink-0 rounded-full"></div>
							<div class="min-w-0 flex-1">
								<div class="text-km-base-content text-sm font-medium">Place {i + 1}</div>
								<div class="text-km-muted-content text-xs">0.{i + 1} km away</div>
							</div>
						</div>
					{/each}
				</div>
  ...
  {/snippet}
		...
		>
			<Drawer title="Filters" description="Adjust your search filters.">
				...
				{/snippet}
				<p class="text-km-base-content text-sm">Filter controls would go here.</p>
				{#snippet footer()}
					<button
