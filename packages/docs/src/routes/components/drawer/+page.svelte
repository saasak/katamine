<script lang="ts">
  ...
  const propsData = [
    ...
    { name: "description", type: "string", default: "—", description: "Drawer description text below the title" },
    { name: "direction", type: '"left" | "right" | "top" | "bottom"', default: '"bottom"', description: "Side the drawer slides in from" },
    { name: "snapPoints", type: "(number | string)[]", default: "—", description: "Snap points as fractions (0–1) or pixel strings" },
    { name: "defaultSnapPoint", type: "number | string", default: "—", description: "Initial snap point when the drawer opens" },
    { name: "onSnapPointChange", type: "(details) => void", default: "—", description: "Callback when the active snap point changes" },
    { name: "snapToSequentialPoints", type: "boolean", default: "false", description: "Force sequential snapping between snap points" },
    { name: "modal", type: "boolean", default: "true", description: "Whether the drawer blocks outside interaction" },
    ...
    { name: "role", type: '"dialog" | "alertdialog"', default: '"dialog"', description: "Semantic role for the drawer" },
...
<div class="space-y-8">
  <div>
    ...
    <p class="mt-2 text-km-muted-content">
      Slide-in panel that overlays content from any edge. Built on Ark UI's Drawer with swipe-to-dismiss, snap points, and grabber handle.
    </p>
  </div>
  ...
  <p>This is a bottom sheet drawer.</p>
    ...
    >
      <Drawer title="Bottom Sheet" description="Swipe down to dismiss.">
        ...
        {/snippet}
        <p class="text-sm text-km-base-content">This is a bottom sheet drawer with a grabber handle. Swipe down or click outside to dismiss.</p>
      </Drawer>
    </DemoCard>
      ...
      <Drawer direction="left" title="Navigation">
        ...
        <nav class="space-y-1">
          {#each navItems as item}
            <button class="flex w-full items-center gap-3 rounded-km-field px-3 py-2.5 text-sm text-km-base-content transition-colors hover:bg-km-muted">
              <span>{item.icon}</span>
  ...
  <p>Side panel content.</p>
    ...
    >
      <Drawer direction="right" title="Item Details" description="View and edit item properties.">
        ...
        <div class="space-y-4">
          <div>
            <label class="text-xs font-medium text-km-muted-content">Name</label>
            <p class="text-sm text-km-base-content">Example Item</p>
          </div>
          <div>
            <label class="text-xs font-medium text-km-muted-content">Status</label>
            <p class="text-sm text-km-base-content">Active</p>
          </div>
          <div>
            <label class="text-xs font-medium text-km-muted-content">Created</label>
            <p class="text-sm text-km-base-content">April 12, 2026</p>
          </div>
        </div>
      </Drawer>
    </DemoCard>
  ...
  {/snippet}
    ...
    >
      <Drawer title="Filters" description="Adjust your search filters.">
        ...
        {/snippet}
        <p class="text-sm text-km-base-content">Filter controls would go here.</p>
        {#snippet footer()}
