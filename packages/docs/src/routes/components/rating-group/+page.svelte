<script lang="ts">
  ...
  const propsData = [
    { name: "value", type: "number", default: "—", description: "Controlled value. Supports bind:value" },
    { name: "defaultValue", type: "number", default: "—", description: "Initial value for uncontrolled mode" },
    { name: "count", type: "number", default: "5", description: "Total number of stars" },
    { name: "label", type: "string", default: "—", description: "Display label rendered above the stars" },
    ...
    { name: "readOnly", type: "boolean", default: "false", description: "Make the rating read-only" },
...
<div class="space-y-8">
  ...
  <div>
    ...
    <p class="mt-2 text-km-muted-content">
      Star rating input with support for half-stars, read-only display, and native form submission. Built on Ark UI.
    </p>
  </div>
...
<RatingGroup label="Your rating" bind:value />
<p>Selected: {value ?? 'none'}</p>`}
    >
      <div class="space-y-2">
        <RatingGroup label="Your rating" bind:value={controlledValue} />
        <p class="text-sm text-km-muted-content">
          Selected: <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{controlledValue ?? "none"}</code>
        </p>
      </div>
    </DemoCard>
