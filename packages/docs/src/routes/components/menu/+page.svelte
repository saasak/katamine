<script lang="ts">
  ...
  const propsData = [
    { name: "items", type: "MenuEntry[]", default: "—", description: "Array of menu items, groups, and separators" },
    ...
    { name: "disabled", type: "boolean", default: "false", description: "Disable the menu trigger" },
    { name: "closeOnSelect", type: "boolean", default: "true", description: "Close menu after an item is selected" },
    { name: "loopFocus", type: "boolean", default: "false", description: "Loop keyboard navigation at list boundaries" },
...
<div class="space-y-8">
  <div>
    ...
    <p class="mt-2 text-km-muted-content">
      Dropdown menu for actions and navigation. Supports grouping, separators, and disabled items. Built on Ark UI.
    </p>
  </div>
  ...
  {#snippet trigger()}File{/snippet}
    ...
    >
      <div class="max-w-sm">
        <Menu
          items={fileItems}
          onSelect={(d) => { lastAction = d.value; }}
        >
          {#snippet trigger()}File{/snippet}
        </Menu>
        {#if lastAction}
          <p class="mt-2 text-sm text-km-muted-content">
            Selected: <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{lastAction}</code>
          </p>
        {/if}
      </div>
    </DemoCard>
