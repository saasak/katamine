  ...
  {/snippet}
		...
		>
			<Splitter panels={[{ id: 'left' }, { id: 'right' }]} defaultSize={[50, 50]}>
				{#snippet panel({ id })}
					<div
						class="text-km-muted-content flex h-32 items-center justify-center text-sm font-medium"
					>
						{id}
					</div>
				{/snippet}
  ...
  {/snippet}
		...
		>
			...
			>
				{#snippet panel({ id })}
					<div
						class="text-km-muted-content flex h-24 items-center justify-center text-sm font-medium"
					>
						{id}
					</div>
				{/snippet}
