	...
	const propsData = [
		{
			...
			type: 'string[]',
			default: '—',
			description: 'Controlled paths (drawn strokes). Supports bind:paths'
		},
		{
			name: 'defaultPaths',
			type: 'string[]',
			default: '—',
			description: 'Initial paths for uncontrolled mode'
		},
		{
			name: 'label',
			type: 'string',
			default: '"Signature"',
			description: 'Label text displayed above the pad'
		},
		{
			name: 'name',
			type: 'string',
		...
		},
		{
			...
			default: '—',
			description: 'Drawing options (stroke size, color, pressure simulation)'
		},
		{
			name: 'onDraw',
			type: '(details) => void',
			default: '—',
			description: 'Callback fired while drawing'
		},
		{
			name: 'onDrawEnd',
			type: '(details) => void',
			default: '—',
			description: 'Callback fired when a stroke ends (includes getDataUrl)'
		},
		{
			name: 'class',
			type: 'string',
...
/>`}
		>
			<div class="max-w-md space-y-3">
				...
				/>
				{#if previewUrl}
					<div class="rounded-km-box border-km-base-300 border p-2">
						<p class="text-km-muted-content mb-1 text-xs">Preview:</p>
