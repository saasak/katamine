<script lang="ts">
  ...
  const propsData = [
    ...
    { name: "checked", type: "boolean", default: "—", description: "Controlled checked state. Supports bind:checked" },
    { name: "defaultChecked", type: "boolean", default: "—", description: "Initial state for uncontrolled mode" },
    ...
    >
      <div class="space-y-2">
        ...
        <p class="text-sm text-km-muted-content">
          State: <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{notifications}</code>
        </p>
      </div>
    </DemoCard>
