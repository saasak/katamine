<script lang="ts">
  ...
  const propsData = [
    { name: "label", type: "string", default: "—", description: "Label text displayed above the dropzone" },
    { name: "hint", type: "string", default: '"Drag and drop or click to upload"', description: "Hint text shown inside the dropzone" },
    { name: "name", type: "string", default: "—", description: "Form field name for native submission" },
    { name: "accept", type: "Record<string, string[]> | FileMimeType | FileMimeType[]", default: "—", description: "Accepted file types (MIME types or extensions)" },
    { name: "maxFiles", type: "number", default: "1", description: "Maximum number of files" },
    { name: "maxFileSize", type: "number", default: "Infinity", description: "Maximum file size in bytes" },
    { name: "minFileSize", type: "number", default: "0", description: "Minimum file size in bytes" },
    { name: "allowDrop", type: "boolean", default: "true", description: "Whether to allow drag and drop" },
    { name: "showPreview", type: "boolean", default: "true", description: "Show image previews for accepted files" },
    ...
    { name: "directory", type: "boolean", default: "—", description: "Accept directories (webkit only)" },
    { name: "capture", type: '"user" | "environment"', default: "—", description: "Camera capture mode" },
    { name: "acceptedFiles", type: "File[]", default: "—", description: "Controlled accepted files. Supports bind:acceptedFiles" },
    { name: "defaultAcceptedFiles", type: "File[]", default: "—", description: "Default accepted files for uncontrolled mode" },
    { name: "onFileChange", type: "(details) => void", default: "—", description: "Callback when files change (accepted or rejected)" },
    { name: "onFileAccept", type: "(details) => void", default: "—", description: "Callback when files are accepted" },
    { name: "onFileReject", type: "(details) => void", default: "—", description: "Callback when files are rejected" },
...
/>`}
    >
      <div class="space-y-2">
        ...
        />
        {#if rejectedInfo}
          <p class="text-sm text-km-error">
            Rejected: <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{rejectedInfo}</code>
          </p>
        {/if}
      </div>
    </DemoCard>
    ...
    >
      <div class="space-y-2">
        ...
        <p class="text-sm text-km-muted-content">
          <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{acceptedFiles.length}</code> file(s) selected
        </p>
      </div>
    </DemoCard>
      ...
      <div class="space-y-3">
        <form
          onsubmit={(e) => {
            ...
            formResult = files.length > 0 ? files.map(f => f instanceof File ? f.name : String(f)).join(", ") : "(no file)";
          }}
          class="space-y-3"
        >
          <FileUpload label="Resume" name="resume" />
