<script lang="ts">
  ...
  let controlledValue = $state<import("@ark-ui/svelte/date-picker").DateValue[]>([]);
  let formResult = $state("");

  const propsData = [
    { name: "value", type: "DateValue[]", default: "—", description: "Controlled selected date(s). Supports bind:value" },
    { name: "defaultValue", type: "DateValue[]", default: "—", description: "Initial value for uncontrolled mode" },
    { name: "name", type: "string", default: "—", description: "Form field name for native submission" },
    { name: "placeholder", type: "string", default: "—", description: "Placeholder text in the input" },
    { name: "selectionMode", type: '"single" | "multiple" | "range"', default: '"single"', description: "Date selection mode" },
    { name: "disabled", type: "boolean", default: "false", description: "Disable the date picker" },
    { name: "readOnly", type: "boolean", default: "false", description: "Make the date picker read-only" },
    ...
    { name: "closeOnSelect", type: "boolean", default: "true (false for multiple)", description: "Close calendar after selection" },
    ...
    { name: "numOfMonths", type: "number", default: "1", description: "Number of months to display" },
    { name: "startOfWeek", type: "number", default: "0", description: "First day of the week (0=Sun, 1=Mon, ..., 6=Sat)" },
    { name: "isDateUnavailable", type: "(date, locale) => boolean", default: "—", description: "Function to mark specific dates as unavailable" },
    { name: "locale", type: "string", default: '"en-US"', description: "Locale for date formatting (BCP 47 tag)" },
    { name: "format", type: "(date, details) => string", default: "—", description: "Custom date format function" },
    { name: "onValueChange", type: "(details) => void", default: "—", description: "Callback when selected value changes" },
...
<div class="space-y-8">
  ...
  <section class="space-y-4">
    ...
    >
      <div class="max-w-sm space-y-2">
        ...
        <p class="text-sm text-km-muted-content">
          Selected: <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{controlledValue.length ? controlledValue.map((d) => d.toString()).join(", ") : "(none)"}</code>
        </p>
      </div>
    </DemoCard>
    ...
    <DemoCard
      ...
      code={`import { parseDate } from "@ark-ui/svelte/date-picker";
