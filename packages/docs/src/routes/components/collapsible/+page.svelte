<script lang="ts">
  ...
  const propsData = [
    ...
    { name: "lazyMount", type: "boolean", default: "false", description: "Lazy mount the content (only render when first opened)" },
    { name: "unmountOnExit", type: "boolean", default: "false", description: "Unmount content DOM when collapsed" },
    { name: "class", type: "string", default: "—", description: "Additional CSS classes on the root element" },
    { name: "trigger", type: "Snippet", default: "—", description: "Custom trigger snippet to replace the default label" },
    { name: "children", type: "Snippet", default: "—", description: "Content to display when expanded" },
  ];
</script>
