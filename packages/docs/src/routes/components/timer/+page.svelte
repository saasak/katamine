<script lang="ts">
  ...
  const propsData = [
    { name: "countdown", type: "boolean", default: "false", description: "Count down instead of up" },
    { name: "startMs", type: "number", default: "—", description: "Start value in milliseconds" },
    { name: "targetMs", type: "number", default: "—", description: "Target/stop value in milliseconds" },
    { name: "autoStart", type: "boolean", default: "false", description: "Auto-start on mount" },
    { name: "interval", type: "number", default: "1000", description: "Tick interval in milliseconds" },
    { name: "segments", type: "TimePart[]", default: '["hours","minutes","seconds"]', description: "Time segments to display" },
    { name: "showControls", type: "boolean", default: "true", description: "Show start, pause, resume, and reset controls" },
    { name: "disabled", type: "boolean", default: "false", description: "Disable all interaction" },
    { name: "onTick", type: "(details: TickDetails) => void", default: "—", description: "Callback on each tick" },
    { name: "onComplete", type: "() => void", default: "—", description: "Callback when the timer completes" },
...
<div class="space-y-8">
  ...
  <div>
    ...
    <p class="mt-2 text-km-muted-content">
      Countdown and count-up timer with configurable segments and built-in controls. Built on Ark UI.
    </p>
  </div>
...
{/if}`}
    >
      <div class="space-y-3">
        ...
        />
        {#if completed}
          <p class="text-sm font-medium text-km-primary">Time's up!</p>
        {/if}
        <p class="text-xs text-km-muted-content">Ticks received: {tickCount}</p>
      </div>
    </DemoCard>
