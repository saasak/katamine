	...
	const propsData = [
		{
			...
			type: 'string',
			default: '—',
			description: 'Controlled color value (hex string). Supports bind:value'
		},
		{
			name: 'defaultValue',
			type: 'string',
		...
		},
		{
			...
			type: 'boolean',
			default: 'false',
			description: 'Render picker inline instead of as a popover'
		},
		{
			name: 'format',
			type: '"rgba" | "hsla" | "hsba" | "hexa"',
			default: '—',
		...
		},
		{
			...
			type: 'boolean',
			default: 'false',
			description: 'Close popover when a swatch is selected. Ignored in inline mode'
		},
		{
			name: 'presets',
			type: 'string[]',
			default: '—',
			description: 'Array of preset color hex strings to display as swatches'
		},
		{
			name: 'onValueChange',
			type: '(details) => void',
		...
		},
		{
			...
			type: '(details) => void',
			default: '—',
			description: 'Callback when color adjustment completes'
		},
		{
			name: 'onFormatChange',
			type: '(details) => void',
			default: '—',
			description: 'Callback when color format changes'
		},
		{
			name: 'onOpenChange',
			type: '(details) => void',
			default: '—',
			description: 'Callback when popover open state changes. Ignored in inline mode'
		},
		{
			name: 'class',
			type: 'string',
...
<div class="space-y-8">
	...
	<div>
		...
		<p class="text-km-muted-content mt-2">
			Interactive color selection with area picker, channel sliders, preset swatches, and eyedropper
			support. Built on Ark UI.
		</p>
	</div>
		...
		>
			<div class="max-w-sm space-y-3">
				...
				<p class="text-km-muted-content text-sm">
					Selected: <code class="bg-km-base-200 rounded px-1.5 py-0.5 font-mono text-xs"
