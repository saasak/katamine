<script lang="ts">
  ...
  const propsData = [
    ...
    { name: "value", type: "number[]", default: "—", description: "Controlled value (array for range mode). Supports bind:value" },
    { name: "defaultValue", type: "number[]", default: "[50]", description: "Default value for uncontrolled mode" },
    ...
    { name: "origin", type: '"start" | "center" | "end"', default: '"start"', description: "Origin of the filled range" },
    { name: "disabled", type: "boolean", default: "false", description: "Disable the slider" },
    { name: "readOnly", type: "boolean", default: "false", description: "Make the slider read-only" },
    { name: "invalid", type: "boolean", default: "false", description: "Mark as invalid" },
    { name: "showValue", type: "boolean", default: "false", description: "Show the current value as text" },
    { name: "markers", type: "number[]", default: "—", description: "Show step markers at given values" },
    { name: "minStepsBetweenThumbs", type: "number", default: "0", description: "Minimum steps between thumbs in range mode" },
    { name: "getAriaValueText", type: "(details) => string", default: "—", description: "Function to format the displayed value text" },
    { name: "onValueChange", type: "(details) => void", default: "—", description: "Callback when value changes" },
    { name: "onValueChangeEnd", type: "(details) => void", default: "—", description: "Callback when dragging ends" },
    { name: "onFocusChange", type: "(details) => void", default: "—", description: "Callback when focused thumb changes" },
...
<div class="space-y-8">
  <div>
    ...
    <p class="mt-2 text-km-muted-content">
      A slider control for selecting numeric values from a range. Supports single thumb, range mode, markers, and vertical orientation. Built on Ark UI.
    </p>
  </div>
...
<p>Value: {controlledValue}</p>`}
    >
      <div class="space-y-2">
        ...
        <p class="text-sm text-km-muted-content">
          Value: <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{controlledValue}</code>
        </p>
      </div>
    </DemoCard>
    ...
    >
      <div class="space-y-2">
        ...
        <p class="text-sm text-km-muted-content">
          Range: <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{rangeValue[0]}–{rangeValue[1]}</code>
        </p>
      </div>
    </DemoCard>
