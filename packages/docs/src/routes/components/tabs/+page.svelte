  ...
  {/snippet}
		...
		>
			<Tabs items={basicTabs} defaultValue="account">
				{#snippet content(value)}
					{#if value === 'account'}
						<p class="text-km-base-content text-sm">Manage your account details and preferences.</p>
					{/if}
					{#if value === 'password'}
						<p class="text-km-base-content text-sm">Update your password and security settings.</p>
					{/if}
		...
		>
			<div class="flex">
				<Tabs items={basicTabs} orientation="vertical" defaultValue="account">
					{#snippet content(value)}
						{#if value === 'account'}
							<p class="text-km-base-content pl-4 text-sm">Account panel content.</p>
						{/if}
						{#if value === 'password'}
							<p class="text-km-base-content pl-4 text-sm">Password panel content.</p>
						{/if}
						{#if value === 'notifications'}
							<p class="text-km-base-content pl-4 text-sm">Notifications panel content.</p>
						{/if}
					{/snippet}
				</Tabs>
			</div>
  ...
  {#snippet content(value)}...{/snippet}
		...
		>
			<Tabs items={basicTabs} activationMode="manual" defaultValue="account">
				{#snippet content(value)}
					{#if value === 'account'}
						<p class="text-km-base-content text-sm">Account — activated manually.</p>
					{/if}
					{#if value === 'password'}
						<p class="text-km-base-content text-sm">Password — activated manually.</p>
					{/if}
					{#if value === 'notifications'}
						<p class="text-km-base-content text-sm">Notifications — activated manually.</p>
					{/if}
				{/snippet}
			</Tabs>
		</DemoCard>
  ...
  {#snippet content(value)}...{/snippet}
		...
		>
			<Tabs items={withDisabled} defaultValue="general">
				{#snippet content(value)}
					{#if value === 'general'}
						<p class="text-km-base-content text-sm">General settings.</p>
					{/if}
					{#if value === 'security'}
						<p class="text-km-base-content text-sm">Security settings (unreachable).</p>
					{/if}
					{#if value === 'billing'}
						<p class="text-km-base-content text-sm">Billing and payment info.</p>
					{/if}
				{/snippet}
			</Tabs>
		</DemoCard>
			...
			<div class="space-y-3">
				...
				>
					<Tabs items={basicTabs} name="tab" defaultValue="account">
						{#snippet content(value)}
							{#if value === 'account'}
								<p class="text-km-base-content text-sm">Account tab selected.</p>
							{/if}
							{#if value === 'password'}
								<p class="text-km-base-content text-sm">Password tab selected.</p>
							{/if}
							{#if value === 'notifications'}
								<p class="text-km-base-content text-sm">Notifications tab selected.</p>
							{/if}
						{/snippet}
					</Tabs>
					<button
				...
				{#if formResult}
					<p class="text-km-muted-content text-sm">
						Submitted: <code class="bg-km-base-200 rounded px-1.5 py-0.5 font-mono text-xs"
							>{formResult}</code
						>
					</p>
				{/if}
