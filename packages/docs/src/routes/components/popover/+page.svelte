    ...
    <input type="text" value="100%" class="rounded-km-field border border-km-base-300 bg-km-base-100 px-3 py-1.5 text-sm" />
		...
		>
			<Popover title="Dimensions" description="Set the dimensions for the layer.">
				{#snippet trigger()}
					<button
						class="rounded-km-field bg-km-primary text-km-primary-content px-4 py-2 text-sm font-medium"
					>
						Open popover
					</button>
				{/snippet}
				<div class="grid gap-2">
					<label class="text-km-base-content text-sm">Width</label>
					<input
						type="text"
						value="100%"
						class="rounded-km-field border-km-base-300 bg-km-base-100 border px-3 py-1.5 text-sm"
					/>
				</div>
			</Popover>
		</DemoCard>
			...
			<div class="flex gap-4">
				<Popover title="Top" positioning={{ placement: 'top' }}>
					{#snippet trigger()}
						...
						>
					{/snippet}
					<p class="text-km-muted-content text-sm">Popover positioned on top.</p>
				</Popover>
				<Popover title="Right" positioning={{ placement: 'right' }}>
					{#snippet trigger()}
						...
						>
					{/snippet}
					<p class="text-km-muted-content text-sm">Popover positioned on the right.</p>
				</Popover>
				<Popover title="Bottom" positioning={{ placement: 'bottom' }}>
					{#snippet trigger()}
						...
						>
					{/snippet}
					<p class="text-km-muted-content text-sm">Popover positioned on the bottom.</p>
				</Popover>
			</div>
		</DemoCard>
    ...
    <button class="...">Submit</button>
		...
		>
			<Popover title="Feedback" description="How are we doing?">
				{#snippet trigger()}
					<button
						class="rounded-km-field bg-km-secondary text-km-secondary-content px-4 py-2 text-sm font-medium"
					>
						Give feedback
					</button>
				{/snippet}
				<div class="grid gap-3">
					...
					></textarea>
					<button
						class="rounded-km-field bg-km-primary text-km-primary-content px-3 py-1.5 text-sm font-medium"
					>
						Submit
					</button>
				</div>
  ...
  <p>Popover content here.</p>
		...
		>
			<Popover title="Welcome" description="This popover opens by default." defaultOpen={true}>
				...
				{/snippet}
				<p class="text-km-muted-content text-sm">Popover content here.</p>
			</Popover>
		</DemoCard>
	</section>
