			...
			<div class="max-w-lg">
				<Steps items={basicSteps}>
					{#snippet content(index)}
						...
						{:else if index === 1}
							<p class="text-km-base-content text-sm">Tell us a bit about yourself.</p>
						{:else}
							<p class="text-km-base-content text-sm">Review your information before submitting.</p>
						{/if}
					{/snippet}
				</Steps>
			</div>
		...
		>
			<div class="max-w-lg">
				<Steps items={onboardingSteps}>
					{#snippet content(index)}
						<p class="text-km-base-content text-sm">Content for step {index + 1}.</p>
					{/snippet}
					{#snippet completed()}
						<div class="text-center">
							<p class="text-km-primary text-lg font-semibold">All done!</p>
							<p class="text-km-muted-content text-sm">You've completed the onboarding wizard.</p>
						</div>
					{/snippet}
				</Steps>
			</div>
		...
		>
			<div class="max-w-lg">
				<Steps items={basicSteps} disabled={true}>
					{#snippet content(index)}
						<p class="text-km-base-content text-sm">Step {index + 1} is locked.</p>
					{/snippet}
				</Steps>
			</div>
		</DemoCard>
  ...
  {/snippet}
		...
		>
			<div class="max-w-lg space-y-3">
				...
				>
					{#snippet content(index)}
						<p class="text-km-base-content text-sm">You are on step {index + 1}.</p>
					{/snippet}
				</Steps>
				{#if formResult}
					<p class="text-km-muted-content text-sm">
						Event: <code class="bg-km-base-200 rounded px-1.5 py-0.5 font-mono text-xs"
							>{formResult}</code
						>
					</p>
				{/if}
