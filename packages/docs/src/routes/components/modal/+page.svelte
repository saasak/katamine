<script lang="ts">
  ...
  const propsData = [
    ...
    { name: "description", type: "string", default: "—", description: "Modal description text below the title" },
    { name: "variant", type: '"responsive" | "drawer"', default: '"responsive"', description: "Display mode. 'responsive' shows a dialog on desktop and drawer on mobile. 'drawer' always shows a bottom-sheet drawer." },
    { name: "breakpoint", type: "number", default: "768", description: "Viewport width in px below which the drawer is used (only for variant='responsive')" },
    { name: "modal", type: "boolean", default: "true", description: "Whether the modal blocks outside interaction" },
    { name: "closeOnEscape", type: "boolean", default: "true", description: "Close on Escape key press" },
    { name: "closeOnInteractOutside", type: "boolean", default: "true", description: "Close when clicking outside" },
    { name: "preventScroll", type: "boolean", default: "true", description: "Prevent background scrolling when open" },
    { name: "role", type: '"dialog" | "alertdialog"', default: '"dialog"', description: "Semantic role for the modal" },
    { name: "snapPoints", type: "(number | string)[]", default: "—", description: "Snap points for the drawer as fractions (0–1) or pixel strings. Ignored in dialog mode." },
    { name: "defaultSnapPoint", type: "number | string", default: "—", description: "Initial snap point when the drawer opens. Ignored in dialog mode." },
    { name: "onSnapPointChange", type: "(details) => void", default: "—", description: "Callback when the active snap point changes. Ignored in dialog mode." },
    { name: "snapToSequentialPoints", type: "boolean", default: "false", description: "Force sequential snapping between snap points. Ignored in dialog mode." },
...
<div class="space-y-8">
  <div>
    ...
    <p class="mt-2 text-km-muted-content">
      Responsive modal that renders as a centered dialog on desktop and a bottom-sheet drawer on mobile. Built on Ark UI Dialog + Drawer primitives.
    </p>
  </div>
  ...
  <p>This renders as a dialog on desktop and a drawer on mobile.</p>
    ...
    >
      <Modal title="Welcome" description="This is a responsive modal.">
        ...
        {/snippet}
        <p class="text-sm text-km-base-content">This renders as a dialog on desktop and a drawer on mobile. Try resizing your browser!</p>
      </Modal>
    </DemoCard>
  ...
  <p>This is always a bottom-sheet drawer.</p>
    ...
    >
      <Modal variant="drawer" title="Bottom Sheet" description="Always a drawer.">
        ...
        {/snippet}
        <p class="text-sm text-km-base-content">This is always a bottom-sheet drawer, even on desktop. Swipe down to dismiss.</p>
      </Modal>
    </DemoCard>
  ...
  <p>Drag the handle to resize.</p>
    ...
    >
      ...
      >
        ...
        {/snippet}
        <div class="space-y-3">
          <p class="text-sm text-km-base-content">This drawer snaps to 25%, 50%, and 100% of the viewport height — like Google Maps.</p>
          <div class="space-y-2">
            {#each Array(8) as _, i}
              <div class="flex items-center gap-3 rounded-km-field border border-km-base-300 p-3">
                <div class="h-10 w-10 shrink-0 rounded-full bg-km-muted"></div>
                <div class="min-w-0 flex-1">
                  <div class="text-sm font-medium text-km-base-content">Place {i + 1}</div>
                  <div class="text-xs text-km-muted-content">0.{i + 1} km away</div>
                </div>
              </div>
            {/each}
          </div>
...
<Modal bind:open title="Controlled Modal">
  <p>Opened via external state.</p>
    ...
    >
      ...
      <Modal bind:open={controlledOpen} title="Controlled Modal" description="This modal is controlled via bind:open.">
        <p class="text-sm text-km-base-content">Opened via external state. Close me with the X button, Escape key, or swipe down on mobile.</p>
      </Modal>
    </DemoCard>
  </section>
