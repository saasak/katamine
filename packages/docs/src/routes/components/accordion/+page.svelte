<script lang="ts">
  ...
  const faqItems = [
    { value: "what", label: "What is Katamine?" },
  ...
  const faqContent: Record<string, string> = {
    what: "Katamine is a CLI tool that copies ready-to-use, themed SvelteKit components into your project. Built on Ark UI primitives.",
    install: "Run npx katamine init to scaffold your project, then npx katamine add <component> to add individual components.",
    theme: "Yes! Katamine uses semantic CSS variables (OKLCH) that you can override. Switch themes via the data-theme attribute on your HTML element.",
    components: "Katamine includes Accordion, Collapsible, ComboBox, and more. Each component is fully composed and ready to use out of the box.",
  };

  let controlledValue = $state<string[]>(["what"]);

  const propsData = [
    { name: "items", type: "T[]", default: "—", description: "Array of items with value, label, and optional disabled" },
    { name: "value", type: "string[]", default: "—", description: "Controlled expanded item(s). Supports bind:value" },
    { name: "defaultValue", type: "string[]", default: "—", description: "Initial expanded item(s) for uncontrolled mode" },
    { name: "multiple", type: "boolean", default: "false", description: "Allow multiple items open simultaneously" },
    { name: "collapsible", type: "boolean", default: "true", description: "Allow collapsing an open item by clicking again" },
    { name: "disabled", type: "boolean", default: "false", description: "Disable all accordion items" },
    { name: "orientation", type: '"horizontal" | "vertical"', default: '"vertical"', description: "Orientation of the accordion" },
    { name: "onValueChange", type: "(details) => void", default: "—", description: "Callback when expanded items change" },
    { name: "onFocusChange", type: "(details) => void", default: "—", description: "Callback when focused item changes" },
    { name: "class", type: "string", default: "—", description: "Additional CSS classes on the root element" },
    { name: "children", type: "Snippet<[T]>", default: "—", description: "Content snippet — receives the current item" },
  ];
</script>
    ...
    >
      <div class="max-w-lg space-y-3">
        ...
        <p class="text-sm text-km-muted-content">
          Open: <code class="rounded bg-km-base-200 px-1.5 py-0.5 font-mono text-xs">{controlledValue.join(", ") || "(none)"}</code>
        </p>
      </div>
    </DemoCard>
