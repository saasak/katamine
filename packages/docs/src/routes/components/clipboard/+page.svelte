<script lang="ts">
  ...
  const propsData = [
    { name: "value", type: "string", default: "—", description: "Controlled value to copy. Supports bind:value" },
    { name: "defaultValue", type: "string", default: "—", description: "Initial value for uncontrolled mode" },
    { name: "timeout", type: "number", default: "3000", description: "Duration in ms to show the copied indicator" },
    { name: "label", type: "string", default: "—", description: "Optional label displayed above the input" },
    { name: "disabled", type: "boolean", default: "false", description: "Disable the clipboard input and trigger" },
    { name: "onValueChange", type: "(details: { value: string }) => void", default: "—", description: "Callback when the clipboard value changes" },
    { name: "onStatusChange", type: "(details: { copied: boolean }) => void", default: "—", description: "Callback when copy status changes" },
...
<div class="space-y-8">
  ...
  <section class="space-y-4">
    ...
    <DemoCard
      ...
      code={`<Clipboard value="https://katamine.dev" />`}
    >
      <div class="max-w-sm">
        <Clipboard value="https://katamine.dev" />
      </div>
    </DemoCard>
    ...
    <DemoCard
      ...
      code={`<Clipboard value="npm install katamine" label="Install command" />`}
    >
      <div class="max-w-sm">
        <Clipboard value="npm install katamine" label="Install command" />
      </div>
    </DemoCard>
...
/>`}
    >
      <div class="max-w-sm space-y-2">
        ...
        />
        {#if statusMessage}
          <p class="text-sm text-km-primary">{statusMessage}</p>
        {/if}
      </div>
    </DemoCard>
