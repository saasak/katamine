		...
		>
			<div class="max-w-lg">
				<Carousel items={images}>
					{#snippet children(image)}
						<img src={image.src} alt={image.alt} class="rounded-km-box w-full object-cover" />
					{/snippet}
				</Carousel>
			</div>
		</DemoCard>
		...
		>
			<div class="max-w-lg">
				<Carousel items={cards} slidesPerPage={2} spacing="16px" loop>
					{#snippet children(card)}
						<div class="rounded-km-box border-km-base-300 bg-km-base-200 border p-4">
							<h3 class="text-km-base-content font-semibold">{card.title}</h3>
							<p class="text-km-muted-content mt-1 text-sm">{card.description}</p>
						</div>
					{/snippet}
				</Carousel>
			</div>
		...
		>
			<div class="max-w-lg space-y-3">
				...
				<p class="text-km-muted-content text-sm">
					Current page: <code class="bg-km-base-200 rounded px-1.5 py-0.5 font-mono text-xs"
						>{controlledPage}</code
					>
				</p>
			</div>
