<script lang="ts">
  ...
  const propsData = [
    ...
    { name: "page", type: "number", default: "—", description: "Controlled current page index. Supports bind:page" },
    { name: "defaultPage", type: "number", default: "—", description: "Initial page for uncontrolled mode" },
    { name: "orientation", type: '"horizontal" | "vertical"', default: '"horizontal"', description: "Layout direction" },
    { name: "autoplay", type: "boolean", default: "false", description: "Auto-advance slides" },
    { name: "autoplayDelay", type: "number", default: "4000", description: "Autoplay interval in milliseconds" },
    { name: "loop", type: "boolean", default: "false", description: "Wrap around to first/last slide" },
    { name: "slidesPerPage", type: "number", default: "1", description: "Number of slides visible at once" },
    { name: "spacing", type: "string", default: '"0px"', description: "Gap between slides (CSS value)" },
    { name: "showIndicators", type: "boolean", default: "true", description: "Show dot indicators" },
    { name: "showControls", type: "boolean", default: "true", description: "Show prev/next arrow buttons" },
    { name: "disabled", type: "boolean", default: "false", description: "Disable all interaction" },
    { name: "onPageChange", type: "(details) => void", default: "—", description: "Callback when the active page changes" },
    { name: "class", type: "string", default: "—", description: "Additional CSS classes on the root element" },
    { name: "children", type: "Snippet<[T, number]>", default: "—", description: "Render snippet for each slide — receives item and index" },
  ];
</script>
...
<div class="space-y-8">
  ...
  <div>
    ...
    <p class="mt-2 text-km-muted-content">
      Interactive slideshow for cycling through content. Supports autoplay, looping, and multiple slides per page. Built on Ark UI.
    </p>
  </div>
  ...
  {#snippet children(image)}
    <img src={image.src} alt={image.alt} class="w-full rounded-km-box object-cover" />
  {/snippet}
</Carousel>`}
    >
      <div class="max-w-lg">
  ...
  {#snippet children(card)}
    <div class="rounded-km-box border border-km-base-300 bg-km-base-200 p-4">
      <h3 class="font-semibold text-km-base-content">{card.title}</h3>
      <p class="mt-1 text-sm text-km-muted-content">{card.description}</p>
    </div>
  {/snippet}
</Carousel>`}
    >
