<script lang="ts">
  ...
  const propsData = [
    ...
    { name: "itemToLabel", type: "(item: T) => string", default: "item.label", description: "Extract display label from item" },
    { name: "itemToValue", type: "(item: T) => string", default: "item.value", description: "Extract unique value from item" },
    { name: "value", type: "string[]", default: "—", description: "Controlled selected value(s). Supports bind:value" },
    { name: "defaultValue", type: "string[]", default: "—", description: "Initial value(s) for uncontrolled mode" },
    ...
    { name: "closeOnSelect", type: "boolean", default: "!multiple", description: "Close dropdown after selection" },
    { name: "onSearch", type: "(query: string) => Promise<T[]> | T[]", default: "—", description: "Async search handler. Enables async mode when provided" },
    { name: "debounceMs", type: "number", default: "300", description: "Debounce delay for async search in ms" },
    { name: "filter", type: '"contains" | "startsWith" | function', default: '"contains"', description: "Sync filter strategy or custom function" },
    { name: "onValueChange", type: "(details) => void", default: "—", description: "Callback when selected value(s) change" },
    { name: "onInputValueChange", type: "(details) => void", default: "—", description: "Callback when input text changes" },
