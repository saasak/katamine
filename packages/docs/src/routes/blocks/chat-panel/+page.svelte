<script lang="ts">
  ...
  let messages: ChatMessage[] = $state([
    { id: "1", content: "Hey! How's the project going?", sender: "Alice", avatar: "AL", timestamp: "2:30 PM" },
    { id: "2", content: "Going well! Just finished the new component system.", sent: true, timestamp: "2:31 PM", status: "read" },
    { id: "3", content: "That's awesome. Can you show me a demo?", sender: "Alice", avatar: "AL", timestamp: "2:32 PM" },
    { id: "4", content: "Sure, I'll send a link in a sec.", sent: true, timestamp: "2:33 PM", status: "delivered" },
  ]);
  ...
  const statusMessages: ChatMessage[] = [
    { id: "1", content: "Testing message statuses", sent: true, timestamp: "1:00 PM", status: "sending" },
    { id: "2", content: "This one was sent", sent: true, timestamp: "1:01 PM", status: "sent" },
    { id: "3", content: "This was delivered", sent: true, timestamp: "1:02 PM", status: "delivered" },
    { id: "4", content: "And this was read", sent: true, timestamp: "1:03 PM", status: "read" },
  ];

  const chatPanelProps = [
    { name: "messages", type: "ChatMessage[]", default: "[]", description: "Array of messages to display" },
    { name: "onSend", type: "(message: string) => void", default: "—", description: "Callback when a message is submitted" },
    { name: "title", type: "string", default: "—", description: "Header title (e.g. contact name)" },
    { name: "subtitle", type: "string", default: "—", description: "Header subtitle (e.g. \"Online\")" },
    ...
    { name: "header", type: "Snippet", default: "—", description: "Custom header snippet (overrides title/subtitle/headerAvatar)" },
  ...
  const chatBubbleProps = [
    ...
    { name: "timestamp", type: "string", default: "—", description: "Timestamp string (e.g. \"2:34 PM\")" },
    { name: "sender", type: "string", default: "—", description: "Sender name (shown for received messages)" },
    { name: "avatar", type: "string", default: "—", description: "Avatar initials (e.g. \"JD\")" },
    { name: "sent", type: "boolean", default: "false", description: "Whether this message was sent by the current user" },
    { name: "status", type: '"sending" | "sent" | "delivered" | "read"', default: "—", description: "Message delivery status (sent messages only)" },
...
<div class="space-y-8">
  <div>
    ...
    <p class="mt-2 text-km-muted-content">
      A complete chat interface with message bubbles, auto-scrolling message area, and an input with send button. Responsive with a 350px minimum width.
    </p>
  </div>
