---
name: add-component
description: Add a new Katamine component (Ark UI wrapper + docs page + nav entry)
user-invocable: true
allowed-tools: Read, Edit, Write, Glob, Grep, Bash(pnpm:*), Bash(npx:*), Bash(mise:*), Agent, Skill(svelte-autofixer:*)
---

# Add Katamine Component

You are adding a new component to the Katamine UI library. Follow every phase in order. Do NOT skip any phase or present the result until ALL validation checks pass.

## Phase 1 — Input & Research

1. Parse `$ARGUMENTS` as the component name (e.g., `accordion`). Derive `<PascalName>` from it (e.g., `Accordion`).
2. Use the Svelte MCP `list-sections` tool to find all relevant Ark UI / SvelteKit documentation sections.
3. Use `get-documentation` to fetch the Ark UI docs for this component. Identify:
   - The Ark UI import path (e.g., `@ark-ui/svelte/accordion`)
...
- Wrap Ark UI primitives into a **single ready-to-use component** with a clean props interface
...
- Forward `name` prop for native form submission (Ark/Zag handles hidden input)
...
- A11y delegated entirely to Ark/Zag — **no manual ARIA attributes**
- Icons from `lucide-svelte`
- **Tailwind classes only** — use `km-*` tokens exclusively:
  - Surfaces: `bg-km-base-100`, `bg-km-base-200`
  - Text: `text-km-base-content`, `text-km-muted-content`
  - Borders: `border-km-base-300`
  - Focus: `focus-within:border-km-primary`
  - Hover: `hover:bg-km-muted`
  - Active/selected: `text-km-primary`, `bg-km-primary text-km-primary-content`
  - Radius: `rounded-km-selector` (checkboxes/toggles/badges), `rounded-km-field` (inputs/buttons/tabs), `rounded-km-box` (cards/dropdowns/modals)
  - Shadows: `shadow-km-sm`, `shadow-km-md`, `shadow-km-lg`
  - Z-index: `z-[var(--km-z-dropdown)]` etc.
  - Transitions: use `duration-150` inline
- Use `Portal` + `Positioner` for floating content (dropdowns, popovers, tooltips)
